#!/bin/bash
aws cloudformation list-exports --query 'Exports[?Name==`brave-new-world-security-'"$STAGE"'`].Value' --output text > rolearn.txt
aws cloudformation list-exports --query 'Exports[?Name==`brave-new-world-security-secrets-'"$STAGE"'`].Value' --output text > s3.txt
